...
public class PathWrapperDeserializer extends JsonDeserializer<PathWrapper> {
    ...
    public PathWrapper deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
        return createPathWrapper((ObjectMapper) p.getCodec(), p.readValueAsTree(), false, true, false);
    }

    public static PathWrapper createPathWrapper(ObjectMapper objectMapper, JsonNode path, boolean tmpElevation, boolean turnDescription, boolean pointsEncoded) {
        ...
        if (path.has("points")) {
            ...
            if (pointsEncoded) {
                ...
                pointList = WebHelper.decodePolyline(pointStr, 100, tmpElevation);
                pathWrapper.setPoints(pointList);
            } else {
                LineString lineString = objectMapper.convertValue(path.get("points"), LineString.class);
                pointList = PointList.fromLineString(lineString);
                pathWrapper.setPoints(pointList);
            }
