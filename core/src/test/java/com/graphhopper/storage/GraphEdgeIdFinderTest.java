...
public class GraphEdgeIdFinderTest {
    ...
    public void testParseStringHints() {
        ...
        GraphEdgeIdFinder.BlockArea blockArea = graphFinder.parseBlockArea("0.01,0.005,1", DefaultEdgeFilter.allEdges(encoder.getAccessEnc()), 1000 * 1000);
        ...
        blockArea = graphFinder.parseBlockArea("0,0,1000", DefaultEdgeFilter.allEdges(encoder.getAccessEnc()), 1000 * 1000);
        blockedEdges.clear();
        assertEquals(blockedEdges, blockArea.blockedEdges);
    ...
    public void testBlockAreasWithPolygon() {
        ...
        GraphEdgeIdFinder graphFinder = new GraphEdgeIdFinder(graph, locationIndex);
        GraphEdgeIdFinder.BlockArea blockArea = graphFinder.parseBlockArea("2,1, 0,2, 2,3", DefaultEdgeFilter.allEdges(encoder.getAccessEnc()), 1000 * 1000);

        GHIntHashSet blockedEdges = new GHIntHashSet();
        blockedEdges.addAll(1, 2, 6, 7);
        assertEquals(blockedEdges, blockArea.blockedEdges);

        blockArea = graphFinder.parseBlockArea("2,1, 1,3, 1,2, 0,1", DefaultEdgeFilter.allEdges(encoder.getAccessEnc()), 1000 * 1000);
